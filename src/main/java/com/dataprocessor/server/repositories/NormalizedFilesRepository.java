...
@Service
public class NormalizedFilesRepository {
    private final Logger logger = LoggerFactory.getLogger(getClass());
    private final MinioClient minioClient;
    private final String bucket;

    @Autowired
    public NormalizedFilesRepository(@Value("${minio.endpoint}") final String uri,
                                     @Value("${minio.user}") final String userName,
                                     @Value("${minio.password}") final String password,
                                     @Value("${minio.buckets.normalized}") final String bucket) throws ServerException, InsufficientDataException, ErrorResponseException, IOException, NoSuchAlgorithmException, InvalidKeyException, InvalidResponseException, XmlParserException, InternalException {
        this.bucket = bucket;
    ...
    public final void saveNormalized(final String name, final File file){
        try {
    ...
    public final File getNormalized(final String name){
        try {
            final File res = TempFileUtil.createTmpFile(".csv");
